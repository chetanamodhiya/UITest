...
public class ChangeHandlingFeeChangetoSmallQuantityFeeCQA_269 extends TestBase{
	...
	public void handlingFeetoSmallQuantityFee(Hashtable<String,String> data) throws Throwable{
		Map<String,String> run=excelUtility.readExcelCellValueGivenTheKey(PropertiesUtility.getPropertyValueByKey("CQA_269_excelPath"), PropertiesUtility.getPropertyValueByKey("sheetForRun"));
		...
		pdp.clickStartOverLink("Start");
		pdp.selectAnOptionInAllTheDropdowns();
		pdp.addQuantity(data.get("Quantity"));	
		pdp.clickProceedToShippingButton();
		set.validateTextAsPerQuantity(data.get("Quantity"),data.get("Expected Charge"));
	}
	
	@DataProvider
	public Object[][] getData(Method m) throws EncryptedDocumentException, IOException{
		String sheetName = m.getName();
		int rows = excelUtility.getRowCount(PropertiesUtility.getPropertyValueByKey("CQA_269_excelPath"), sheetName);
		int columns = excelUtility.getColumnCount(PropertiesUtility.getPropertyValueByKey("CQA_269_excelPath"), sheetName);
		rows = rows+1;
		...
		for(int rowNum = 2; rowNum <= rows; rowNum++) {
			...
			for(int colNum = 0; colNum < columns; colNum++) {
				table.put(excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("CQA_269_excelPath"), sheetName, 1, colNum), excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("CQA_269_excelPath"), sheetName, rowNum, colNum));
