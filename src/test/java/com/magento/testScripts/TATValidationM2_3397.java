...
public class TATValidationM2_3397 extends TestBase{
...
public static FileInputStream fis;
	...
	public void tATValidation(Hashtable<String,String> data) throws Throwable{
		Map<String,String> run=excelUtility.readExcelCellValueGivenTheKey(PropertiesUtility.getPropertyValueByKey("TATExcel"), PropertiesUtility.getPropertyValueByKey("sheetForRun"));
		Map<String,String> files=excelUtility.readExcelCellValueGivenTheKey(PropertiesUtility.getPropertyValueByKey("TATExcel"), PropertiesUtility.getPropertyValueByKey("sheetForFiles"));
		...
		if(PropertiesUtility.getPropertyValueByKey("env").equalsIgnoreCase("dev"))
			...
			navigateTo_custom(data.get("ProductURL").replace("envi", run.get("PrintingKeyword")));
		pdp.clickStartOverLink("Start Over");
//		pdp.tATValidationWithSizeAndRunsize_M2_3397(data.get("Products"),files.get("Quantity"),sa);
		pdp.tATValidationWithAllOptionsAndRunsize_M2_3397(data.get("Products"),files.get("Quantity"),sa);
		sa.assertAll();
	}
	...
	public Object[][] getData(Method m) throws EncryptedDocumentException, IOException{
		...
		int rows = excelUtility.getRowCount(PropertiesUtility.getPropertyValueByKey("TATExcel"), sheetName);
		int columns = excelUtility.getColumnCount(PropertiesUtility.getPropertyValueByKey("TATExcel"), sheetName);
		rows = rows+1;
		...
		for(int rowNum = 2; rowNum <= rows; rowNum++) {
			...
			for(int colNum = 0; colNum < columns; colNum++) {
				table.put(excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("TATExcel"), sheetName, 1, colNum), excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("TATExcel"), sheetName, rowNum, colNum));
