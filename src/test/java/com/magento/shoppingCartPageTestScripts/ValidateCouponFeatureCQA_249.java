...
public class ValidateCouponFeatureCQA_249 extends TestBase{
	...
	public void validateCouponCodeOnShoppingCartPage() throws Throwable{
		...
		//try to store the 'Cost summary' details into a map > map1
		Map<String, String> costSummaryWithoutApplyingCoupon=cart.costSummaryStoredInMap();
		cart.clickApplyPromoOrRewardsLabel();
		cart.applyCouponCode(couponDetails.get("CouponFor$1"));
		cart.verifyDiscountIsApplied(couponDetails.get("DiscountFor$1"));
		
		//try to store the 'Cost summary' details into a map > map2
		Map<String, String> costSummaryWithApplyingCoupon=cart.costSummaryStoredInMap();
		cart.verifyCostSummary(costSummaryWithoutApplyingCoupon,costSummaryWithApplyingCoupon,sa);
		cart.clickRemovePromoCodeButton();
		cart.verifyDiscountIsRemoved(sa);
		//try to store the 'Cost summary' details into a map > map3
		Map<String, String> costSummaryAfterRemovingCoupon=cart.costSummaryStoredInMap();
		cart.verifyCostSummary(costSummaryWithoutApplyingCoupon,costSummaryAfterRemovingCoupon,sa);
		cart.applyCouponCode(couponDetails.get("CouponFor1%"));
		cart.verifyDiscountIsApplied(MathUtility.parseDoubleToString(MathUtility.percentageCount(cart.viewSubTotalPrice())));
		costSummaryWithApplyingCoupon=cart.costSummaryStoredInMap();
		cart.verifyCostSummary(costSummaryWithoutApplyingCoupon,costSummaryWithApplyingCoupon,sa);
		cart.clickRemovePromoCodeButton();
		cart.verifyCostSummary(costSummaryWithoutApplyingCoupon,costSummaryAfterRemovingCoupon,sa);
		cart.verifyDiscountIsRemoved(sa);
//		//try to store the 'Cost summary' details into a map > map4
		sa.assertAll();
	}
}