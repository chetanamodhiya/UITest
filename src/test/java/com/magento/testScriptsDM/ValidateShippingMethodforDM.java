package com.magento.testScriptsDM;
...
public class ValidateShippingMethodforDM extends TestBase{
	...
	public void validateShippingMethod(Hashtable<String,String> data) throws Throwable{
		Map<String,String> run=excelUtility.readExcelCellValueGivenTheKey(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), PropertiesUtility.getPropertyValueByKey("sheetForRun"));
		Map<String,String> fileUploading=excelUtility.readExcelCellValueGivenTheKey(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), PropertiesUtility.getPropertyValueByKey("sheetForFiles"));
...
public Object[][] getData(Method m) throws EncryptedDocumentException, IOException{
	...
	int rows = excelUtility.getRowCount(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), sheetName);
	int columns = excelUtility.getColumnCount(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), sheetName);
	rows = rows+1;
	...
	for(int rowNum = 2; rowNum <= rows; rowNum++) {
		...
		for(int colNum = 0; colNum < columns; colNum++) {
			table.put(excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), sheetName, 1, colNum), excelUtility.getCellData(PropertiesUtility.getPropertyValueByKey("ShippingMethodvalidationForDMExcel"), sheetName, rowNum, colNum));
